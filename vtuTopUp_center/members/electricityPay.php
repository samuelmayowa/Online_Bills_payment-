...
<!DOCTYPE  html>
<html>
<head><title>-Electricity-VtuTopCenter</title>
	<meta  charset="utf-8">
...
if(isset($_GET['data'])){ 
  if ($_GET['data']=="success"){
	...
	$successReport="Your request to pay  for  ".$product. " was successful for this amount : ₦". $amount. ".";
	} else {
		$successReport="Electricity payment request  was  successful. But something went wrong with the URL";
	}
 ...
 if(isset($_GET['functionError'])){ 
    ...
    switch($_GET['error']){ 
      ...
      case "lowAmount":
            ...
            $errorMsg="Request completed, but admin was not informed. Kindly inform admin about your electric bill pay :". $api_error;
            break;
           default:
         ...
         <aside id="dasboard">
         	...
         	<div class="main-dashboard"> 
         		<div class="alternate_inner-div">
					...
					<h1 class='reflink'>Electricity Bill: </h1>
				...
				<!-- discount for MTN: 2.00%  GLO: 2.05%  Airtel: 2.05%  9Mobile: 2.05%     -->			
					 <div id="registration-form"> <button id='wallet_funding'>Make Purchase</button>
					 	 <button id='wallet_history'>Purchase History</button> <br/><br/> 
					 
					 	 	<div class='fieldset' >

					 	 		<form action="../enclosed/electricityPay.inc" method="post" id="wallet-form" data-validate="">
							<div class='row'>
								<label for='Disco'>Select  Disco: </label> 
								 <select id="disco"  name='disco' required > 
                                		<option value="">-- Select --</option>
                             			<?php loadSub_product('24'); ?>
                                        
                                      </select>
							</div><br/>
							<div class='row'>
								<label for='meter Number'>Meter Number:</label> 
							<input type="text"  name='meter_no' id="meter_no"  value=" "/> 
							</div><br/>
							<div class='row'>
								<label for='Mobile Number'>Customer Mobile Number:</label> 
							<input type="number"   name='mobile' id="mobile"  value="" /> 
							</div><br/>
							<div class='row'>
								<label for='Amount'>Enter Amount:</label> 
							<input type="number"   name='amount' id="amount"  min="1000" max="100000"value="" /> 
							<p id='hint'>NOTE: Minimum electricity purchase is ₦1000.00 and Maximum is ₦100,000.00</p>
							</div>
							<div class='row'>
								<label for='Amount to pay'>Amount To Pay:</label> 
							<input type="text"  readonly name='amountToPay' id="amountToPay"  value="" /> 
							</div>
							<input type="submit" name='payElectric' value="Make Payment" onClick='return confirmSubmit()'>
						</form>
					</div>
<script LANGUAGE="JavaScript">
		...
		var agree=confirm("You are about to pay Electric Bill  worth of  " +amount+ ",  press -ok- to confirm else press  -cancel- ");
		if (agree)
		 return true ;
		else
		 return false ;
...
<div class='wallet_table' >
	<table border="1px" id="table">
		<thead>
	<tr><th> SN</th> <th>Product</th><th>Old Balance</th> <th>Amount Deducted</th> <th>New Balance</th>  <th>Token</th> 
		  <th>Date</th><th>Status</th>
			...
			<tbody>
				<?php
			 	include "../condb.inc/connection.php";
$sql="SELECT * FROM electrichistory WHERE User_id='$u_id' ORDER BY  electric_id DESC";
				$query=mysqli_query($con,$sql);
				if(!empty($query)){
					$sn=0;
			...
			<tr>
			<td><?php echo $sn ; ?></td><td><?php echo $product; ?> </td><td>&#8358;<?php echo $oldBaln; ?></td><td>&#8358;<?php echo $amount_deducted; ?></td><td>&#8358;<?php echo $new_baln; ?></td><td><?php echo $token; ?> </td>
			<td><?php echo $date; ?></td><td><?php echo $status; ?></td>
			</tr>
