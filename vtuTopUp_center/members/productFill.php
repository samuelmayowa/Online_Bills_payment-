...
//lets insert in  data into category table
	$sql="INSERT INTO products (cat_code ,products,prd_code )
	VALUES('$cat_code',	'$product','$prod_code')";  
	$query=mysqli_query($con,$sql);
	if(mysqli_affected_rows($con)==1){
		echo "<div  class='showSuccess'>Product insertted.</div>";
		//exit();
	}else{
		echo "<div  class='showSuccess'>Product was not inserted.</div>";
	}
         ...
         <aside id="dasboard">
         	...
         	<div class="main-dashboard"> 
         		<div class="alternate_inner-div">
					...
					<h1 class='reflink'>Fill the Products  table: </h1>
					
							
					 <div id="registration-form"> 
					 	 <button id='wallet_history'>For products table</button>
					 	 	<div class='fieldset' >
					 	 		<form action="" method="post" id="wallet-form" data-validate="">
							<div class='row'>
								<label for='categoryCode'>category Code:</label> 
								<input type="number" placeholder="" name='cat_code' id="cat_code" /> 
								
							</div><br/>
							<div class='row'>
								<label for='Products'>Products:</label> 
								<input type="text" placeholder="" name='product' id="cat_code" /> 
								
							</div><br/>
							<div class='row'>
								<label for='prod_code'>Product_code:</label> 
								<input type="number" placeholder="" name='product_code' id="cat_code" /> 
							</div>
