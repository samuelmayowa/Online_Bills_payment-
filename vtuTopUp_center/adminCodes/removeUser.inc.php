...
if(empty($trnxinfo) ||  empty($mobile) ||  empty($id)){
	$status=false;
	header("location:../alegbeleye/removeuser?error=emptyFields&user=$mobile");
	exit();
}
...
if($mobile==$mainAdmin){
	$status=false;
	header("location:../alegbeleye/removeuser?error=isMainAdmin&user=$mobile");
	exit();
}
...
$processed=removeUser($mobile,$id);
...
$mail->Subject = 'VtuTopCenter Removed You';
// format the message
...
$message.="<p>Sorry, to infrom you that you have been delisted from VtuTopCenter. </p>";
$message.="<p><b>Purpose of removal : </b><br/>$trnxinfo </p>";
$message.="<br/>";
...
//Email subject
$mailUser->Subject = 'A User Removed ';
// format the message
$message="<h1>This is to inform the admin that a user has been removed from the system.</h1>";
...
header("location:../alegbeleye/seeUser?remove=success&user=$mobile");
	exit();

}else{
	$status=false;
	header("location:../alegbeleye/removeduser?error=annoucefailed&user=$mobile");
	exit();
