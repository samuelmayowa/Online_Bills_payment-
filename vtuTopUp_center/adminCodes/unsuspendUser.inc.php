...
if(empty($trnxinfo) ||  empty($mobile) ||  empty($id)){
	$status=false;
	header("location:../alegbeleye/unsuspenduser?error=emptyFields&user=$mobile");
	exit();
}
...
if($mobile==$mainAdmin){
	$status=false;
	header("location:../alegbeleye/unsuspenduser?error=isMainAdmin&user=$mobile");
	exit();
}
...
// lets even check if the user is on suspension in the first place.
 $is_suspended=is_suspended($mobile);
 if($is_suspended!==true){
	$status=false;
	header("location:../alegbeleye/unsuspenduser?error=notSuspended&user=$mobile");
	exit();

 }
// lets now usee funticon to unsuspend this user
$processed=unSuspendUser($mobile);
...
$mail->Subject = 'VtuTopCenter Un-suspended You';
// format the message
...
$message.="<p>We like to infrom you that you have been un-suspended on VtuTopCenter and now have full access to our services. </p>";
$message.="<p><b>Purpose of restoration : </b><br/>$trnxinfo </p>";
$message.="<br/>";
...
//Email subject
$mailUser->Subject = 'A User Un suspended ';
// format the message
$message="<h1>This is to inform the admin that a suspended user has been restored back to the system.</h1>";
$message.="<p>See few details of the user..</p>";
$message.="<p><b>Email:</b> $UserEmail </p>";
$message.="<p><b>User Phone Number:</b> $mobile </p>";
$message.="<p><b>Purpose of restoration:</b> $trnxinfo </p>";
$message.="<p><b>Admin responsible:</b> $adminName </p>";
$mailUser->Body =$message;
$ok_mailUser=$mailUser->send();	


header("location:../alegbeleye/seeUser?restore=success&user=$mobile");
	exit();

}else{
	$status=false;
	header("location:../alegbeleye/unsuspenduser?error=annoucefailed&user=$mobile");
	exit();
